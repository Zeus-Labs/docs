...
If you are deploying with Option 1, ZeusCloud expects your AWS configurations in `~/.aws/config` and `~/.aws/credentials`. If your configurations are in a different location, update the `AWS_CONFIG_FILE` and `AWS_SHARED_CREDENTIALS_FILE` variables in the `.env` file. For Windows, change to `%USERPROFILE%\.aws\config` and `%USERPROFILE%\.aws\credentials`, respectively.
